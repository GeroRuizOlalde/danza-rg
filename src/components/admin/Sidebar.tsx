...
export default function Sidebar() {
  ...
  useEffect(() => {
    async function fetchSidebarData() {
      ...
      // 2. Cargar cantidad de turnos pendientes
      ...
      if (!error && data) {
        setTurnosPendientes(data.length);
      }
    }

    fetchSidebarData();

    // 3. (Opcional pero genial) Escuchar cambios en la tabla de reservas 
    // para actualizar la burbuja roja si llega un turno nuevo sin recargar la página.
    const canalReservas = supabase.channel('cambios-reservas')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'reservas' }, () => {
        // Si hay algún cambio en reservas, volvemos a contar
  ...
  // Función para desloguear 100% seguro
  const handleLogout = async () => {
    await supabase.auth.signOut();
    document.cookie = "rg-admin-auth=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT"; // Borrar cookie del middleware
    router.push("/admin/login");
  };

  // Generar las iniciales (Ej: Gerónimo -> GE)
  const iniciales = adminName.slice(0, 2).toUpperCase();

  const navItems = [
    { section: "Principal", links: [{ name: "Dashboard", path: "/admin/dashboard", icon: "🏠" }] },
  ...
  return (
    <aside className="w-[240px] h-screen bg-[#1A1A22] flex-shrink-0 flex flex-col relative overflow-hidden">
      {/* Círculo fondo */}
      <div className="absolute -bottom-[100px] -left-[100px] w-[300px] h-[300px] bg-[radial-gradient(circle,rgba(232,160,180,0.07)_0%,transparent_65%)] pointer-events-none"></div>
          ...
          <div key={idx}>
            ...
            {group.links.map((link) => {
              ...
              return (
                ...
                >
                  <span className={`w-[18px] h-[18px] flex items-center justify-center text-[15px] ${isActive ? "text-[#E8A0B4]" : ""}`}>
                    {link.icon}
                  </span>
                  <span>{link.name}</span>
                  {link.badge && (
      ...
      <div className="p-5 border-t border-white/5 z-10">
        <div className="flex items-center gap-3 p-2 -mx-2 rounded-xl hover:bg-white/5 transition-colors group">
          ...
          <button onClick={handleLogout} className="text-white/25 hover:text-[#E8A0B4] text-[13px] transition-colors" title="Cerrar sesión">
            ⏻
          </button>
        </div>
      </div>
