...
export default function ClientesPage() {
  ...
  const handleEliminarAlumna = async (alumna: Alumna, e?: React.MouseEvent) => {
    e?.stopPropagation();
    const confirmar = window.confirm(
      `⚠️ ¿Estás segura de eliminar a ${alumna.nombre} ${alumna.apellido}?\n\nEsta acción eliminará su perfil, inscripciones y datos asociados. No se puede deshacer.`
    );
    if (!confirmar) return;

    setEliminandoId(alumna.id);
    const result = await eliminarAlumnaAction(alumna.id);
    setEliminandoId(null);

    if (result.success) {
      toast.success(`${alumna.nombre} ${alumna.apellido} fue eliminada.`);
      setExpandedId(null);
      await fetchClientes();
    } else {
    ...
    <div className="font-dm-sans min-h-screen">
      ...
      {cargando ? (
        ...
        <div className="bg-white rounded-2xl border border-[#E8A0B4]/20 shadow-sm overflow-hidden">
          ...
          <div className="hidden lg:grid grid-cols-[44px_1fr_180px_140px_120px_130px] gap-0 px-5 py-2.5 bg-[#F7F7F9] border-b border-[#E8A0B4]/20">
            <div />
              ...
              {grupo.map((c, idx) => {
                ...
                return (
                  <div key={c.id} className={`${!isLast ? "border-b border-gray-50" : ""}`}>
                    {/* Fila principal — mobile: card / desktop: grid row */}
                    <div
                      onClick={() => setExpandedId(isExpanded ? null : c.id)}
                      className={`flex items-center gap-3 px-4 py-3.5 lg:grid lg:grid-cols-[44px_1fr_180px_140px_120px_130px] lg:gap-0 lg:px-5 cursor-pointer transition-all duration-150 hover:bg-[#FDF0F4]/40 ${isExpanded ? "bg-[#FDF0F4]/50" : ""}`}
                    >
                      {/* Avatar */}
                      ...
                      {/* Nombre + badge mobile */}
                      <div className="flex flex-col justify-center min-w-0 flex-1 lg:pr-4">
                        <div className="flex items-center gap-2">
                          <span className="text-[0.88rem] font-semibold text-[#1A1A22] truncate">
                            {c.apellido}, {c.nombre}
                          </span>
                          <span className={`lg:hidden text-[0.6rem] font-bold px-2 py-0.5 rounded-full border shrink-0 ${badge.cls}`}>
                            {badge.label}
                          </span>
                        </div>
                        <span className="text-[0.75rem] text-[#8A8A99] truncate">{c.email || c.telefono || "Sin contacto"}</span>
                      </div>

                      {/* Contacto (solo desktop) */}
                      <div className="hidden lg:flex flex-col justify-center min-w-0 pr-3">
                      ...
                      {/* Chevron mobile */}
                      <span className="lg:hidden text-[#8A8A99] text-xs shrink-0">{isExpanded ? "▲" : "▼"}</span>
                    </div>

                    {/* Panel expandido */}
                    {isExpanded && (
                      <div className="px-4 lg:px-5 pb-5 pt-1 bg-[#F7F7F9]/60 border-t border-[#E8A0B4]/10">
                        <div className="lg:ml-[44px] grid grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
                          ...
                          <div className="bg-white rounded-xl p-3.5 border border-[#E8A0B4]/15 shadow-sm">
                            <p className="text-[0.6rem] font-black uppercase tracking-widest text-[#8A8A99] mb-1">Teléfono</p>
                            <p className="text-[0.8rem] font-medium text-[#1A1A22]">{c.telefono || "—"}</p>
                          </div>
                          <div className="bg-white rounded-xl p-3.5 border border-[#E8A0B4]/15 shadow-sm">
                            ...
                            <p className="text-[0.8rem] font-medium text-[#1A1A22]">
                              {c.fecha_nacimiento
                                ? `${new Date(c.fecha_nacimiento + "T12:00:00").toLocaleDateString("es-AR")} (${calcularEdad(c.fecha_nacimiento)})`
                                : "—"}
                            </p>
                          </div>
                          <div className="bg-white rounded-xl p-3.5 border border-[#E8A0B4]/15 shadow-sm">
                        ...
                        {/* Documentación (visible en mobile ya que la tabla no la muestra) */}
                        <div className="lg:ml-[44px] flex items-center gap-2 mt-3 lg:hidden">
                          <button
                            onClick={(e) => toggleCheckbox(c.id, "apto_medico", c.apto_medico, e)}
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[0.72rem] font-bold border transition-all ${
                              c.apto_medico
                                ? "bg-emerald-50 text-emerald-600 border-emerald-100"
                                : "bg-red-50 text-red-400 border-red-100"
                            }`}
                          >
                            {c.apto_medico ? "✓" : "✕"} Apto médico
                          </button>
                          <button
                            onClick={(e) => toggleCheckbox(c.id, "fotocopia_dni", c.fotocopia_dni, e)}
                            className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-[0.72rem] font-bold border transition-all ${
                        ...
                        {/* Botones expandidos */}
                        <div className="lg:ml-[44px] flex flex-wrap gap-2 mt-3">
                          <button
                          ...
                          <button
                            onClick={(e) => handleEliminarAlumna(c, e)}
                            disabled={eliminandoId === c.id}
                            className="inline-flex items-center gap-1.5 bg-white border border-red-200 text-red-500 px-4 py-2 rounded-full text-[0.75rem] font-bold hover:bg-red-500 hover:text-white hover:border-red-500 transition-colors disabled:opacity-50 ml-auto"
                          >
